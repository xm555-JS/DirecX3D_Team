...
void CPhysX_Manager::Create_TriAngleMesh(const PxVec3** pVertices, _uint iNumVertice, const PxU32** pIndices, _uint iNumIndice, PxTriangleMesh** ppOut)
{
	PxTriangleMeshDesc MeshDesc;
	MeshDesc.points.count = iNumVertice;
	MeshDesc.triangles.count = iNumIndice / 3;
	MeshDesc.points.stride = sizeof(PxVec3);
	MeshDesc.triangles.stride = sizeof(PxU32) * 3;
	MeshDesc.points.data = &(*pVertices)[0];
	MeshDesc.triangles.data = &(*pIndices)[0];
	MeshDesc.flags = PxMeshFlags(0);


	PxDefaultMemoryOutputStream	Stream;
	m_pCooking->cookTriangleMesh(MeshDesc, Stream);
	...
	*ppOut = m_pPhysics->createTriangleMesh(rb);
