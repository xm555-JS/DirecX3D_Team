#include "..\Public\VIBuffer_Terrain_Custom.h"

CVIBuffer_Terrain_Custom::CVIBuffer_Terrain_Custom()
	: CVIBuffer()
{
}

CVIBuffer_Terrain_Custom::CVIBuffer_Terrain_Custom(const CVIBuffer_Terrain_Custom& rhs)
	: CVIBuffer(rhs)
...
HRESULT CVIBuffer_Terrain_Custom::Initialize_Prototype(const _tchar* pHeightMapFilePath)
{
	_ulong			dwByte = 0;
...
#pragma region VERTEXBUFFER
	...
	for (_uint i = 0; i < m_iNumVerticesZ; ++i)
	{
		for (_uint j = 0; j < m_iNumVerticesX; ++j)
		{
			...
			pVertices[iIndex].vPosition = m_pVerticesPos[iIndex] = _float3(j * 4.f, (pPixel[iIndex] & 0x000000ff) / 1.f, i * 4.f);
...
HRESULT CVIBuffer_Terrain_Custom::Initialize(void* pArg)
{
	return S_OK;
}

CVIBuffer_Terrain_Custom* CVIBuffer_Terrain_Custom::Create(const _tchar* pHeightMapFilePath)
{
	CVIBuffer_Terrain_Custom* pInstance = new CVIBuffer_Terrain_Custom();

	if (FAILED(pInstance->Initialize_Prototype(pHeightMapFilePath)))
	{
		MSG_BOX("Failed to Created : CVIBuffer_Terrain_Custom");
...
CComponent* CVIBuffer_Terrain_Custom::Clone(void* pArg)
{
	CVIBuffer_Terrain_Custom* pInstance = new CVIBuffer_Terrain_Custom(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned : CVIBuffer_Terrain_Custom");
		Safe_Release(pInstance);
	}
...
void CVIBuffer_Terrain_Custom::Free()
{
	__super::Free();
}
