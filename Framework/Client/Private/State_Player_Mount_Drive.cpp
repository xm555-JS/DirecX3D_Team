#include "stdafx.h"
#include "..\Public\State_Player_Mount_Drive.h"
...
CState_Player_Mount_Drive::CState_Player_Mount_Drive(CState_Manager* pManager, STATE_TYPE eState)
	: CState_Player(pManager, eState)
{
}

CState_Player_Mount_Drive::CState_Player_Mount_Drive(const CState_Player_Mount_Drive& rhs)
	: CState_Player(rhs)
{
}

HRESULT CState_Player_Mount_Drive::Initialize_Prototype()
{
	...
	m_ReserveStates.push_back(STATE_PLAYER_MOUNT_ROLL);

	return S_OK;
}

HRESULT CState_Player_Mount_Drive::Initialize(void* pArg)
{
	return S_OK;
}

void CState_Player_Mount_Drive::Enter(void* pArg)
{
	if (m_pState_Handler->Get_PreStateType() == STATE_PLAYER_MOUNT_IDLE)
	{
	...
	EFFECT_EXCUTE("MotorCycle_M",
		XMMatrixTranslation(0.165f, 0.2f, -1.45f),
		1.f,
...
void CState_Player_Mount_Drive::Tick(_float fTimeDelta)
{
...
void CState_Player_Mount_Drive::Exit(void* pArg)
{
...
_bool CState_Player_Mount_Drive::Check_Condition(CState_Handler* pHandler)
{
	USEGAMEINSTANCE;
...
void CState_Player_Mount_Drive::Set_PlayerLook(_float fTimeDelta)
{
	USEGAMEINSTANCE;
	if (KEY_DOWN('W') ||
		KEY_DOWN('A') ||
...
CState_Player_Mount_Drive* CState_Player_Mount_Drive::Create(CState_Manager* pManager, STATE_TYPE eState)
{
	CState_Player_Mount_Drive* pInstance = new CState_Player_Mount_Drive(pManager, eState);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created : CState_Player_Mount_Drive");
...
CState* CState_Player_Mount_Drive::Clone(void* pArg)
{
	CState_Player_Mount_Drive* pInstance = new CState_Player_Mount_Drive(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned : CState_Player_Mount_Drive");
		Safe_Release(pInstance);
	}
...
void CState_Player_Mount_Drive::Free()
{
}
