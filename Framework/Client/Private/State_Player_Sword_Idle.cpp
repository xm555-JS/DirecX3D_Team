#include "stdafx.h"
#include "..\Public\State_Player_Sword_Idle.h"
...
CState_Player_Sword_Idle::CState_Player_Sword_Idle(CState_Manager* pManager, STATE_TYPE eState)
	: CState_Player(pManager, eState)
{
}

CState_Player_Sword_Idle::CState_Player_Sword_Idle(const CState_Player_Sword_Idle& rhs)
	: CState_Player(rhs)
{
}

HRESULT CState_Player_Sword_Idle::Initialize_Prototype()
{
...
HRESULT CState_Player_Sword_Idle::Initialize(void* pArg)
{
	return S_OK;
}

void CState_Player_Sword_Idle::Enter(void* pArg)
{
	__super::Enter(pArg);
	m_fAnimSpeed = 2.f;
}

void CState_Player_Sword_Idle::Tick(_float fTimeDelta)
{
...
void CState_Player_Sword_Idle::Exit(void* pArg)
{
	m_fKeyPressTimeAcc = 0.f;
	__super::Exit();
}

_bool CState_Player_Sword_Idle::Check_Condition(CState_Handler* pHandler)
{
	USEGAMEINSTANCE;
	if (KEY_NONE('W') &&
		KEY_NONE('A') &&
...
CState_Player_Sword_Idle* CState_Player_Sword_Idle::Create(CState_Manager* pManager, STATE_TYPE eState)
{
	CState_Player_Sword_Idle* pInstance = new CState_Player_Sword_Idle(pManager, eState);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created : CState_Player_Sword_Idle");
...
CState* CState_Player_Sword_Idle::Clone(void* pArg)
{
	CState_Player_Sword_Idle* pInstance = new CState_Player_Sword_Idle(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned : CState_Player_Sword_Idle");
		Safe_Release(pInstance);
	}
...
void CState_Player_Sword_Idle::Free()
{
}
