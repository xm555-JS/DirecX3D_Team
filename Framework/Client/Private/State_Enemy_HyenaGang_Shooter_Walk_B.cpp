#include "stdafx.h"
#include "..\Public\State_Enemy_HyenaGang_Shooter_Walk_B.h"
#include "GameInstance.h"
#include "Enemy.h"
CState_Enemy_HyenaGang_Shooter_Walk_B::CState_Enemy_HyenaGang_Shooter_Walk_B(CState_Manager* pManager, STATE_TYPE eState)
	:CState_Enemy_Normal(pManager, eState)
{
}

CState_Enemy_HyenaGang_Shooter_Walk_B::CState_Enemy_HyenaGang_Shooter_Walk_B(const CState_Enemy_HyenaGang_Shooter_Walk_B& rhs)
	: CState_Enemy_Normal(rhs)
{
}

HRESULT CState_Enemy_HyenaGang_Shooter_Walk_B::Initialize_Prototype()
{
...
HRESULT CState_Enemy_HyenaGang_Shooter_Walk_B::Initialize(void* pArg)
{
	m_fBackEndDistance = 3.5f;
	return S_OK;
}

void CState_Enemy_HyenaGang_Shooter_Walk_B::Enter(void* pArg)
{
	__super::Enter(pArg);
...
void CState_Enemy_HyenaGang_Shooter_Walk_B::Tick(_float fTimeDelta)
{
	if (Is_Damaged())
		m_bEndCurState = true;
...
void CState_Enemy_HyenaGang_Shooter_Walk_B::Exit(void* pArg)
{

}

_bool CState_Enemy_HyenaGang_Shooter_Walk_B::Check_Condition(CState_Handler* pHandler)
{
...
CState_Enemy_HyenaGang_Shooter_Walk_B* CState_Enemy_HyenaGang_Shooter_Walk_B::Create(CState_Manager* pManager, STATE_TYPE eState)
{
	CState_Enemy_HyenaGang_Shooter_Walk_B* pInstance = new CState_Enemy_HyenaGang_Shooter_Walk_B(pManager, eState);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created : CState_Enemy_HyenaGang_Shooter_Walk_B");
...
CState* CState_Enemy_HyenaGang_Shooter_Walk_B::Clone(void* pArg)
{
	CState_Enemy_HyenaGang_Shooter_Walk_B* pInstance = new CState_Enemy_HyenaGang_Shooter_Walk_B(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned : CState_Enemy_HyenaGang_Shooter_Walk_B");
		Safe_Release(pInstance);
	}
...
void CState_Enemy_HyenaGang_Shooter_Walk_B::Free()
{
	__super::Free();
}
