#include "stdafx.h"
#include "..\Public\State_Player_Mount_Roll_Air.h"
...
CState_Player_Mount_Roll_Air::CState_Player_Mount_Roll_Air(CState_Manager* pManager, STATE_TYPE eState)
	: CState_Player(pManager, eState)
{
}

CState_Player_Mount_Roll_Air::CState_Player_Mount_Roll_Air(const CState_Player_Mount_Roll_Air& rhs)
	: CState_Player(rhs)
{
}

HRESULT CState_Player_Mount_Roll_Air::Initialize_Prototype()
{
	...
	m_ReserveStates.push_back(STATE_PLAYER_MOUNT_DRIVE_AIR);
	m_ReserveStates.push_back(STATE_PLAYER_MOUNT_IDLE);						

	return S_OK;
}

HRESULT CState_Player_Mount_Roll_Air::Initialize(void* pArg)
{
	return S_OK;
}

void CState_Player_Mount_Roll_Air::Enter(void* pArg)
{
	...
	EFFECT_EXCUTE("MotorCycle_Dash_M",
		XMMatrixRotationRollPitchYaw(XMConvertToRadians(-30.f), 0.f, 0.f) * XMMatrixTranslation(0.165f, 0.f, -1.04f),
		1.f,
...
void CState_Player_Mount_Roll_Air::Tick(_float fTimeDelta)
{
...
void CState_Player_Mount_Roll_Air::Exit(void* pArg)
{
...
_bool CState_Player_Mount_Roll_Air::Check_Condition(CState_Handler* pHandler)
{
	USEGAMEINSTANCE;
...
CState_Player_Mount_Roll_Air* CState_Player_Mount_Roll_Air::Create(CState_Manager* pManager, STATE_TYPE eState)
{
	CState_Player_Mount_Roll_Air* pInstance = new CState_Player_Mount_Roll_Air(pManager, eState);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created : CState_Player_Mount_Roll_Air");
...
CState* CState_Player_Mount_Roll_Air::Clone(void* pArg)
{
	CState_Player_Mount_Roll_Air* pInstance = new CState_Player_Mount_Roll_Air(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned : CState_Player_Mount_Roll_Air");
		Safe_Release(pInstance);
	}
...
void CState_Player_Mount_Roll_Air::Free()
{
}
