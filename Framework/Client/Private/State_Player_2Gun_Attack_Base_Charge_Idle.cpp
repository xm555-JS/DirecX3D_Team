#include "stdafx.h"
#include "..\Public\State_Player_2Gun_Attack_Base_Charge_Idle.h"
...
CState_Player_2Gun_Attack_Base_Charge_Idle::CState_Player_2Gun_Attack_Base_Charge_Idle(CState_Manager* pManager, STATE_TYPE eState)
	: CState_Player(pManager, eState)
{
}

CState_Player_2Gun_Attack_Base_Charge_Idle::CState_Player_2Gun_Attack_Base_Charge_Idle(const CState_Player_2Gun_Attack_Base_Charge_Idle& rhs)
	: CState_Player(rhs)
{
}

HRESULT CState_Player_2Gun_Attack_Base_Charge_Idle::Initialize_Prototype()
{
...
HRESULT CState_Player_2Gun_Attack_Base_Charge_Idle::Initialize(void* pArg)
{
	return S_OK;
}

void CState_Player_2Gun_Attack_Base_Charge_Idle::Enter(void* pArg)
{
	__super::Enter(pArg);
...
void CState_Player_2Gun_Attack_Base_Charge_Idle::Tick(_float fTimeDelta)
{
	USEGAMEINSTANCE;
...
void CState_Player_2Gun_Attack_Base_Charge_Idle::Exit(void* pArg)
{
	m_pColCom = ((CCollider*)m_pState_Handler->Get_Player()->Get_Component(ID_UINT(COLLIDER, COLLIDER_SPHERE, COL_PLAYER_ATTACK)));
	__super::Exit();
}

_bool CState_Player_2Gun_Attack_Base_Charge_Idle::Check_Condition(CState_Handler* pHandler)
{
	USEGAMEINSTANCE;
	if (KEY_NONE('W') &&
			 KEY_NONE('A') &&
...
CState_Player_2Gun_Attack_Base_Charge_Idle* CState_Player_2Gun_Attack_Base_Charge_Idle::Create(CState_Manager* pManager, STATE_TYPE eState)
{
	CState_Player_2Gun_Attack_Base_Charge_Idle* pInstance = new CState_Player_2Gun_Attack_Base_Charge_Idle(pManager, eState);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created : CState_Player_2Gun_Attack_Base_Charge_Idle");
...
CState* CState_Player_2Gun_Attack_Base_Charge_Idle::Clone(void* pArg)
{
	CState_Player_2Gun_Attack_Base_Charge_Idle* pInstance = new CState_Player_2Gun_Attack_Base_Charge_Idle(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned : CState_Player_2Gun_Attack_Base_Charge_Idle");
		Safe_Release(pInstance);
	}
...
void CState_Player_2Gun_Attack_Base_Charge_Idle::Free()
{
}
