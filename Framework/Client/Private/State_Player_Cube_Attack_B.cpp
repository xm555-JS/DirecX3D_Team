#include "stdafx.h"
#include "..\Public\State_Player_Cube_Attack_B.h"
...
CState_Player_Cube_Attack_B::CState_Player_Cube_Attack_B(CState_Manager* pManager, STATE_TYPE eState)
	: CState_Player_Cube(pManager, eState)
{
}

CState_Player_Cube_Attack_B::CState_Player_Cube_Attack_B(const CState_Player_Cube_Attack_B& rhs)
	: CState_Player_Cube(rhs)
{
}

HRESULT CState_Player_Cube_Attack_B::Initialize_Prototype()
{
	...
	m_ReserveStates.push_back(STATE_PLAYER_CUBE_ATTACK_C);
...
HRESULT CState_Player_Cube_Attack_B::Initialize(void* pArg)
{
	return S_OK;
}

void CState_Player_Cube_Attack_B::Enter(void* pArg)
{
	__super::Enter(pArg);
...
void CState_Player_Cube_Attack_B::Tick(_float fTimeDelta)
{
	USEGAMEINSTANCE;
	_matrix ColMatrix;
	...
	Hit_Unit(m_pColCom, ELEMENTAL_ICE, fRandom(200, 220), 11, 20, m_pTargetModel->Get_Animation()[m_pState_Handler->Get_AnimationIdx(m_eState)], &m_bHit0, "Player_Hit_Cube");
	...
	if (11 < Get_CurKeyFrame())
	{
		...
		IMPL_ONCE(m_bEffect0, EFFECT_POS("Player_Cube_Attack2-1", m_pState_Handler->Get_Owner()->Get_WorldMatrix(), XMVectorSet(0.f, 0.f, 3.5f, 1.f)));
...
void CState_Player_Cube_Attack_B::Exit(void* pArg)
{
	m_pColCom = ((CCollider*)m_pState_Handler->Get_Player()->Get_Component(ID_UINT(COLLIDER, COLLIDER_SPHERE, COL_PLAYER_ATTACK)));
...
_bool CState_Player_Cube_Attack_B::Check_Condition(CState_Handler* pHandler)
{
	USEGAMEINSTANCE;
...
CState_Player_Cube_Attack_B* CState_Player_Cube_Attack_B::Create(CState_Manager* pManager, STATE_TYPE eState)
{
	CState_Player_Cube_Attack_B* pInstance = new CState_Player_Cube_Attack_B(pManager, eState);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created : CState_Player_Cube_Attack_B");
...
CState* CState_Player_Cube_Attack_B::Clone(void* pArg)
{
	CState_Player_Cube_Attack_B* pInstance = new CState_Player_Cube_Attack_B(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned : CState_Player_Cube_Attack_B");
		Safe_Release(pInstance);
	}
...
void CState_Player_Cube_Attack_B::Free()
{
}
