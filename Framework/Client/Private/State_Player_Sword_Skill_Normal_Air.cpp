#include "stdafx.h"
#include "..\Public\State_Player_Sword_Skill_Normal_Air.h"
...
CState_Player_Sword_Skill_Normal_Air::CState_Player_Sword_Skill_Normal_Air(CState_Manager* pManager, STATE_TYPE eState)
	: CState_Player(pManager, eState)
{
}

CState_Player_Sword_Skill_Normal_Air::CState_Player_Sword_Skill_Normal_Air(const CState_Player_Sword_Skill_Normal_Air& rhs)
	: CState_Player(rhs)
{
}

HRESULT CState_Player_Sword_Skill_Normal_Air::Initialize_Prototype()
{
...
HRESULT CState_Player_Sword_Skill_Normal_Air::Initialize(void* pArg)
{

	return S_OK;
}

void CState_Player_Sword_Skill_Normal_Air::Enter(void* pArg)
{
	__super::Enter(pArg);
...
void CState_Player_Sword_Skill_Normal_Air::Tick(_float fTimeDelta)
{
	USEGAMEINSTANCE;
	...
	if (61 < m_pTargetModel->Get_Animation()[m_pState_Handler->Get_AnimationIdx(m_eState)]->GetChannel()[0]->Get_NumCurrentKeyFrame())
	{
		m_bEndCurState = true;
	}
	...
	if (3 <= Get_CurKeyFrame())
	{
		...
		IMPL_ONCE(m_bShaked0, pGameInstance->Add_Camera_Shaking(XMVectorSet(0.f, -1.f, 0.f, 0.f), 0.04f, 0.4f, 0.4f));
	}
	...
	if (50 <= Get_CurKeyFrame())
	{
		...
		IMPL_ONCE(m_bShaked4,
			pGameInstance->Add_Camera_Shaking(XMVectorSet(1.f, 0.f, 0.f, 0.f), 0.045f, 0.4f, 0.4f);
			pGameInstance->Add_DirCA(_float2(-1.f, 1.f), _float4(0.008f, 0.f, 0.004f, 0.f), 0.35f, 0.35f);
		);
	}

	Hit_Unit(m_pColCom, ELEMENTAL_LIGHTNING, fRandom(230.f,270.f), 3, 12, m_pTargetModel->Get_Animation()[m_pState_Handler->Get_AnimationIdx(m_eState)], &m_bHit0, "Player_Sword_Hit_Degree0");
	Hit_Unit(m_pColCom, ELEMENTAL_LIGHTNING, fRandom(230.f,270.f), 13, 16, m_pTargetModel->Get_Animation()[m_pState_Handler->Get_AnimationIdx(m_eState)], &m_bHit1,"Player_Sword_Hit_Degree-45");
	Hit_Unit(m_pColCom, ELEMENTAL_LIGHTNING, fRandom(230.f,270.f), 17, 23, m_pTargetModel->Get_Animation()[m_pState_Handler->Get_AnimationIdx(m_eState)], &m_bHit2, "Player_Sword_Hit_Degree45");
	Hit_Unit(m_pColCom, ELEMENTAL_LIGHTNING, fRandom(230.f,270.f), 24, 29, m_pTargetModel->Get_Animation()[m_pState_Handler->Get_AnimationIdx(m_eState)], &m_bHit3, "Player_Sword_Hit_Degree-45");
	Hit_Unit(m_pColCom, ELEMENTAL_LIGHTNING, fRandom(230.f,270.f), 30, 36, m_pTargetModel->Get_Animation()[m_pState_Handler->Get_AnimationIdx(m_eState)], &m_bHit4, "Player_Sword_Hit_Degree45");
	Hit_Unit(m_pColCom, ELEMENTAL_LIGHTNING, fRandom(230.f,270.f), 50, 60, m_pTargetModel->Get_Animation()[m_pState_Handler->Get_AnimationIdx(m_eState)], &m_bHit5, "Player_Sword_Hit_Degree45");
...
void CState_Player_Sword_Skill_Normal_Air::Exit(void* pArg)
{
	m_pColCom = ((CCollider*)m_pState_Handler->Get_Player()->Get_Component(ID_UINT(COLLIDER, COLLIDER_SPHERE, COL_PLAYER_ATTACK)));
	CTransform* pPlayerTransform = (CTransform*)m_pState_Handler->Get_Player()->Get_Component(TRANSFORM);
...
_bool CState_Player_Sword_Skill_Normal_Air::Check_Condition(CState_Handler* pHandler)
{
	USEGAMEINSTANCE;
	if (KEY_DOWN('1'))
	{
...
CState_Player_Sword_Skill_Normal_Air* CState_Player_Sword_Skill_Normal_Air::Create(CState_Manager* pManager, STATE_TYPE eState)
{
	CState_Player_Sword_Skill_Normal_Air* pInstance = new CState_Player_Sword_Skill_Normal_Air(pManager, eState);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created : CState_Player_Sword_Skill_Normal_Air");
...
CState* CState_Player_Sword_Skill_Normal_Air::Clone(void* pArg)
{
	CState_Player_Sword_Skill_Normal_Air* pInstance = new CState_Player_Sword_Skill_Normal_Air(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned : CState_Player_Sword_Skill_Normal_Air");
		Safe_Release(pInstance);
	}
...
void CState_Player_Sword_Skill_Normal_Air::Free()
{
}
