#include "stdafx.h"
#include "..\Public\State_Enemy_Gangster_Cap_Turn_L.h"
#include "GameInstance.h"
#include "Enemy.h"

CState_Enemy_Gangster_Cap_Turn_L::CState_Enemy_Gangster_Cap_Turn_L(CState_Manager* pManager, STATE_TYPE eState)
	:CState_Enemy_Normal(pManager, eState)
{

}

CState_Enemy_Gangster_Cap_Turn_L::CState_Enemy_Gangster_Cap_Turn_L(const CState_Enemy_Gangster_Cap_Turn_L& rhs)
	: CState_Enemy_Normal(rhs)
{
}

HRESULT CState_Enemy_Gangster_Cap_Turn_L::Initialize_Prototype()
{
	...
	m_ReserveProbabilitys[STATE_ENEMY_GANGSTER_CAP_ATTACK_DASH] = 3;
	m_ReserveProbabilitys[STATE_ENEMY_GANGSTER_CAP_ATTACK_SWING] = 3;
	m_ReserveProbabilitys[STATE_ENEMY_GANGSTER_CAP_ATTACK_TURNTURN] = 1;

	m_ReserveProbabilitys[STATE_ENEMY_GANGSTER_CAP_RUN_RETURN] = 5;
	m_ReserveProbabilitys[STATE_ENEMY_GANGSTER_CAP_RUN] = 5;
...
HRESULT CState_Enemy_Gangster_Cap_Turn_L::Initialize(void * pArg)
{

	return S_OK;
}

void CState_Enemy_Gangster_Cap_Turn_L::Enter(void* pArg)
{
	__super::Enter(pArg);
...
void CState_Enemy_Gangster_Cap_Turn_L::Tick(_float fTimeDelta)
{
	if (Is_AnimFinished())
		m_bEndCurState = true;
...
void CState_Enemy_Gangster_Cap_Turn_L::Exit(void* pArg)
{
}

_bool CState_Enemy_Gangster_Cap_Turn_L::Check_Condition(CState_Handler* pHandler)
{
...
CState_Enemy_Gangster_Cap_Turn_L* CState_Enemy_Gangster_Cap_Turn_L::Create(CState_Manager* pManager, STATE_TYPE eState)
{
	CState_Enemy_Gangster_Cap_Turn_L* pInstance = new CState_Enemy_Gangster_Cap_Turn_L(pManager, eState);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created : CState_Enemy_Gangster_Cap_Turn_L");
...
CState* CState_Enemy_Gangster_Cap_Turn_L::Clone(void* pArg)
{
	CState_Enemy_Gangster_Cap_Turn_L* pInstance = new CState_Enemy_Gangster_Cap_Turn_L(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned : CState_Enemy_Gangster_Cap_Turn_L");
		Safe_Release(pInstance);
	}
...
void CState_Enemy_Gangster_Cap_Turn_L::Free()
{
	__super::Free();
}
