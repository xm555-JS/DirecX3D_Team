...
CStructs_Desert::CStructs_Desert()
	: CGameObject()
{
}

CStructs_Desert::CStructs_Desert(const CStructs_Desert& rhs)
	: CGameObject(rhs)
{
}

HRESULT CStructs_Desert::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CStructs_Desert::Initialize(void* pArg)
{
	...
	FAILEDCHECK(__super::Add_Component(m_iCurLevel, ID_UINT(MODEL_STATIC, MODEL_STRUCTS_DESERT, iID.tNums.Num2, iID.tNums.Num3), ID_UINT(MODEL_STATIC), (CComponent**)&m_pModelCom));
...
void CStructs_Desert::Tick(_float fTimeDelta)
{
	if (true == m_bDead)
		return;
}

void CStructs_Desert::LateTick(_float fTimeDelta)
{
	GAMEINSTANCE->Add_RenderGroup(RENDER_NONALPHABLEND, this);
}

HRESULT CStructs_Desert::Render()
{
...
HRESULT CStructs_Desert::SetUp_Components()
{
	/* For.Com_Shader */
...
HRESULT CStructs_Desert::SetUp_ShaderResource()
{
...
void CStructs_Desert::Input_Key(_float fTimeDelta)
{
}

HRESULT CStructs_Desert::SetUp_Model(ID_UINT iPrototypeID)
{
	/* For.Com_Model */
...
CStructs_Desert* CStructs_Desert::Create()
{
	CStructs_Desert* pInstance = new CStructs_Desert();

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created : CStructs_Desert");
...
CGameObject* CStructs_Desert::Clone(void* pArg)
{
	CStructs_Desert* pInstance = new CStructs_Desert(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned : CStructs_Desert");
		Safe_Release(pInstance);
	}
