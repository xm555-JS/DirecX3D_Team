#include "stdafx.h"
#include "..\Public\State_Player_Shield_Attack_Air_A.h"
...
CState_Player_Shield_Attack_Air_A::CState_Player_Shield_Attack_Air_A(CState_Manager* pManager, STATE_TYPE eState)
	: CState_Player(pManager, eState)
{
}

CState_Player_Shield_Attack_Air_A::CState_Player_Shield_Attack_Air_A(const CState_Player_Shield_Attack_Air_A& rhs)
	: CState_Player(rhs)
{
}

HRESULT CState_Player_Shield_Attack_Air_A::Initialize_Prototype()
{
	...
	m_ReserveStates.push_back(STATE_PLAYER_SHIELD_ATTACK_AIR_B);
...
HRESULT CState_Player_Shield_Attack_Air_A::Initialize(void* pArg)
{
	return S_OK;
}

void CState_Player_Shield_Attack_Air_A::Enter(void* pArg)
{
	__super::Enter(pArg);
...
void CState_Player_Shield_Attack_Air_A::Tick(_float fTimeDelta)
{
	USEGAMEINSTANCE;

	if (14 < Get_CurKeyFrame())
	{
		IMPL_ONCE(m_bSound[0], MGR(CSound_Manager)->PlaySurroundSound(TEXT("Shield_Attack_Air_A.wav"), CSound_Manager::CHANNEL_PLAYER, m_pState_Handler->Get_Owner()->Get_Pos(), 0.5f));

		IMPL_ONCE(m_bEffect0,
			EFFECT_POS("Player_Shield_Attack_B", m_pState_Handler->Get_Owner()->Get_WorldMatrix(),
				XMVectorSet(0.f, 0.8f, 0.6f, 1.f), // Pos
	...
	//IMPL_ONCE(m_bHit, );
	Hit_Unit(m_pColCom, ELEMENTAL_FIRE, fRandom(300.f, 350.f), 14, 20, m_pTargetModel->Get_Animation()[m_pState_Handler->Get_AnimationIdx(m_eState)], &m_bHit0, "Player_Hit_ShieldAxe");
...
void CState_Player_Shield_Attack_Air_A::Exit(void* pArg)
{
	CTransform* pPlayerTransform = (CTransform*)m_pState_Handler->Get_Player()->Get_Component(TRANSFORM);
...
_bool CState_Player_Shield_Attack_Air_A::Check_Condition(CState_Handler* pHandler)
{
	USEGAMEINSTANCE;
...
CState_Player_Shield_Attack_Air_A* CState_Player_Shield_Attack_Air_A::Create(CState_Manager* pManager, STATE_TYPE eState)
{
	CState_Player_Shield_Attack_Air_A* pInstance = new CState_Player_Shield_Attack_Air_A(pManager, eState);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created : CState_Player_Shield_Attack_Air_A");
...
CState* CState_Player_Shield_Attack_Air_A::Clone(void* pArg)
{
	CState_Player_Shield_Attack_Air_A* pInstance = new CState_Player_Shield_Attack_Air_A(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned : CState_Player_Shield_Attack_Air_A");
		Safe_Release(pInstance);
	}
...
void CState_Player_Shield_Attack_Air_A::Free()
{
}
