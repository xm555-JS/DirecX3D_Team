#include "stdafx.h"
#include "..\Public\State_Enemy_MechanicalCity_SwordGhost_Run.h"
#include "GameInstance.h"
#include "Enemy.h"
CState_Enemy_MechanicalCity_SwordGhost_Run::CState_Enemy_MechanicalCity_SwordGhost_Run(CState_Manager* pManager, STATE_TYPE eState)
	:CState_Enemy_Normal(pManager, eState)
{
}

CState_Enemy_MechanicalCity_SwordGhost_Run::CState_Enemy_MechanicalCity_SwordGhost_Run(const CState_Enemy_MechanicalCity_SwordGhost_Run& rhs)
...
HRESULT CState_Enemy_MechanicalCity_SwordGhost_Run::Initialize_Prototype()
{
	...
	m_fRunStartDistance = 3.5f; //플레이어와의 거리가 m_fRunStartDistance보다 크다면 실행한다.

	
	return S_OK;
}

HRESULT CState_Enemy_MechanicalCity_SwordGhost_Run::Initialize(void* pArg)
{

	m_fRunEndDistance = 3.5f;

	return S_OK;
}

void CState_Enemy_MechanicalCity_SwordGhost_Run::Enter(void* pArg)
{
	__super::Enter(pArg);
...
void CState_Enemy_MechanicalCity_SwordGhost_Run::Tick(_float fTimeDelta)
{
	if (Is_Damaged())
		m_bEndCurState = true;
...
void CState_Enemy_MechanicalCity_SwordGhost_Run::Exit(void* pArg)
{

}

_bool CState_Enemy_MechanicalCity_SwordGhost_Run::Check_Condition(CState_Handler* pHandler)
{
...
CState_Enemy_MechanicalCity_SwordGhost_Run* CState_Enemy_MechanicalCity_SwordGhost_Run::Create(CState_Manager* pManager, STATE_TYPE eState)
{
	CState_Enemy_MechanicalCity_SwordGhost_Run* pInstance = new CState_Enemy_MechanicalCity_SwordGhost_Run(pManager, eState);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created : CState_Enemy_MechanicalCity_SwordGhost_Run");
...
CState* CState_Enemy_MechanicalCity_SwordGhost_Run::Clone(void* pArg)
{
	CState_Enemy_MechanicalCity_SwordGhost_Run* pInstance = new CState_Enemy_MechanicalCity_SwordGhost_Run(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned : CState_Enemy_MechanicalCity_SwordGhost_Run");
		Safe_Release(pInstance);
	}
...
void CState_Enemy_MechanicalCity_SwordGhost_Run::Free()
{
	__super::Free();
}
