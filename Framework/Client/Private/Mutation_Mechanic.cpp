#include "stdafx.h"
#include "..\Public\Mutation_Mechanic.h"
...
/*돌연변이 메카닉*/
CMutation_Mechanic::CMutation_Mechanic()
	: CEnemy_Normal()
{
}

CMutation_Mechanic::CMutation_Mechanic(const CMutation_Mechanic & rhs)
	: CEnemy_Normal(rhs)
{
}

HRESULT CMutation_Mechanic::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CMutation_Mechanic::Initialize(void * pArg)
{
	m_fFrustumRange = 2.f;
	...
	FAILEDCHECK(Create_UI_Status(11, 1.4f));

	return S_OK;
}

void CMutation_Mechanic::Tick(_float fTimeDelta)
{
#if defined(_TEST_JJ)
	DeadTestKey();
#endif
...
void CMutation_Mechanic::LateTick(_float fTimeDelta)
{
#ifndef _MAPTOOL
...
HRESULT CMutation_Mechanic::Render()
{
...
HRESULT CMutation_Mechanic::SetUp_Components()
{
	FAILEDCHECK(__super::Add_Component(LEVEL_STATIC, ID_UINT(SHADER, SHADER_VTXANIM), ID_UINT(SHADER), (CComponent**)&m_pShaderCom));
	FAILEDCHECK(__super::Add_Component(m_iCurLevel, ID_UINT(MODEL_DYNAMIC, MODEL_ENEMY, MUTATION_MECHANIC), ID_UINT(MODEL_DYNAMIC), (CComponent**)&m_pModelCom));
...
HRESULT CMutation_Mechanic::SetUp_State_Handler()
{
...
CMutation_Mechanic * CMutation_Mechanic::Create()
{
	CMutation_Mechanic*		pInstance = new CMutation_Mechanic();

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created : CMutation_Mechanic");		
...
CGameObject * CMutation_Mechanic::Clone(void * pArg)
{
	CMutation_Mechanic*		pInstance = new CMutation_Mechanic(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned : CMutation_Mechanic");
		Safe_Release(pInstance);
	}
