#include "stdafx.h"
#include "..\Public\State_Enemy_Mutation_Attack_DownHit.h"
#include "GameInstance.h"
#include "Enemy.h"
#include "Effect_Manager.h"
#include "Camera.h"

CState_Enemy_Mutation_Attack_DownHit::CState_Enemy_Mutation_Attack_DownHit(CState_Manager* pManager, STATE_TYPE eState)
	:CState_Enemy_Normal(pManager, eState)
{
}

CState_Enemy_Mutation_Attack_DownHit::CState_Enemy_Mutation_Attack_DownHit(const CState_Enemy_Mutation_Attack_DownHit& rhs)
	: CState_Enemy_Normal(rhs)
{
}

HRESULT CState_Enemy_Mutation_Attack_DownHit::Initialize_Prototype()
{
...
HRESULT CState_Enemy_Mutation_Attack_DownHit::Initialize(void* pArg)
{
	return S_OK;
}

void CState_Enemy_Mutation_Attack_DownHit::Enter(void* pArg)
{
	__super::Enter(pArg);
...
void CState_Enemy_Mutation_Attack_DownHit::Tick(_float fTimeDelta)
{
	if (Is_Damaged())
		m_bEndCurState = true;
	...
	{
		// 포스 회전 스케일
		IMPL_ONCE(m_bEffect1, EFFECT_POS("Mutation_Down", m_pState_Handler->Get_Owner()->Get_WorldMatrix(),
			XMVectorSet(0.f, 0.f, 0.f, 1.f),
...
void CState_Enemy_Mutation_Attack_DownHit::Exit(void* pArg)
{
...
_bool CState_Enemy_Mutation_Attack_DownHit::Check_Condition(CState_Handler* pHandler)
{
...
CState_Enemy_Mutation_Attack_DownHit* CState_Enemy_Mutation_Attack_DownHit::Create(CState_Manager* pManager, STATE_TYPE eState)
{
	CState_Enemy_Mutation_Attack_DownHit* pInstance = new CState_Enemy_Mutation_Attack_DownHit(pManager, eState);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created : CState_Enemy_Mutation_Attack_DownHit");
...
CState* CState_Enemy_Mutation_Attack_DownHit::Clone(void* pArg)
{
	CState_Enemy_Mutation_Attack_DownHit* pInstance = new CState_Enemy_Mutation_Attack_DownHit(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned : CState_Enemy_Mutation_Attack_DownHit");
		Safe_Release(pInstance);
	}
...
void CState_Enemy_Mutation_Attack_DownHit::Free()
{
	__super::Free();
}
