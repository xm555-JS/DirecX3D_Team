...
CBullet_Player_2Gun::CBullet_Player_2Gun()
	: CBullet()
{
}

CBullet_Player_2Gun::CBullet_Player_2Gun(const CBullet_Player_2Gun& rhs)
	: CBullet(rhs)
{
}

HRESULT CBullet_Player_2Gun::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CBullet_Player_2Gun::Initialize(void* pArg)
{
	...
	if(nullptr == ((CPlayer*)m_pOwner)->Get_Target())
		m_pTransformCom->LookTo(pGameInstance->Get_CurrentCamera()->Get_Transform()->Get_WorldMatrix().r[2]);
	else
	{
		m_pTransformCom->LookTo(XMLoadFloat4(&m_BulletDesc.vLookAt));
	}
...
HRESULT CBullet_Player_2Gun::Initialize_Pooling(void* pArg)
{
	USEGAMEINSTANCE;
...
void CBullet_Player_2Gun::Tick(_float fTimeDelta)
{
	if (true == m_bDead)
		return;
	USEGAMEINSTANCE;
...
void CBullet_Player_2Gun::LateTick(_float fTimeDelta)
{
	USEGAMEINSTANCE;
	if (IsInFrustum())
	{
...
HRESULT CBullet_Player_2Gun::Render()
{
...
HRESULT CBullet_Player_2Gun::SetUp_Components()
{
...
CBullet_Player_2Gun* CBullet_Player_2Gun::Create()
{
	CBullet_Player_2Gun* pInstance = new CBullet_Player_2Gun();

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created : CBullet_Player_2Gun");
...
CGameObject* CBullet_Player_2Gun::Clone(void* pArg)
{
	CBullet_Player_2Gun* pInstance = new CBullet_Player_2Gun(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned : CBullet_Player_2Gun");
		Safe_Release(pInstance);
	}
