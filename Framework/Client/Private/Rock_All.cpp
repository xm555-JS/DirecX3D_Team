...
CRock_All::CRock_All(const CRock_All& rhs)
	: CGameObject(rhs)
{
}

HRESULT CRock_All::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CRock_All::Initialize(void* pArg)
{
	...
	FAILEDCHECK(__super::Add_Component(m_iCurLevel, ID_UINT(MODEL_STATIC, MODEL_ROCK_ALL, iID.tNums.Num2, iID.tNums.Num3), ID_UINT(MODEL_STATIC), (CComponent**)&m_pModelCom));
...
void CRock_All::Tick(_float fTimeDelta)
{
	if (true == m_bDead)
		return;
}

void CRock_All::LateTick(_float fTimeDelta)
{
	GAMEINSTANCE->Add_RenderGroup(RENDER_NONALPHABLEND, this);
}

HRESULT CRock_All::Render()
{
...
HRESULT CRock_All::SetUp_Components()
{
	/* For.Com_Shader */
...
HRESULT CRock_All::SetUp_ShaderResource()
{
...
void CRock_All::Input_Key(_float fTimeDelta)
{
}

CRock_All* CRock_All::Create()
{
	CRock_All* pInstance = new CRock_All();

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created : CRock_All");
...
CGameObject* CRock_All::Clone(void* pArg)
{
	CRock_All* pInstance = new CRock_All(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned : CRock_All");
		Safe_Release(pInstance);
	}
