#include "stdafx.h"
#include "..\Public\State_Player_Shield_Skill_Normal.h"
...
CState_Player_Shield_Skill_Normal::CState_Player_Shield_Skill_Normal(CState_Manager* pManager, STATE_TYPE eState)
	: CState_Player(pManager, eState)
{
}

CState_Player_Shield_Skill_Normal::CState_Player_Shield_Skill_Normal(const CState_Player_Shield_Skill_Normal& rhs)
	: CState_Player(rhs)
{
}

HRESULT CState_Player_Shield_Skill_Normal::Initialize_Prototype()
{
...
HRESULT CState_Player_Shield_Skill_Normal::Initialize(void* pArg)
{

	return S_OK;
}

void CState_Player_Shield_Skill_Normal::Enter(void* pArg)
{
	__super::Enter(pArg);
...
void CState_Player_Shield_Skill_Normal::Tick(_float fTimeDelta)
{
	USEGAMEINSTANCE;
	...
	{
		IMPL_ONCE(m_bSound[1], MGR(CSound_Manager)->PlaySurroundSound(TEXT("Axe_Hit_Ground.wav"), CSound_Manager::CHANNEL_PLAYER, m_pState_Handler->Get_Owner()->Get_Pos(), 0.5f));
		...
		IMPL_ONCE(m_bShaked0,
			pGameInstance->Add_Camera_Shaking(XMVectorSet(-1.f, 0.f, 0.f, 0.f), 0.06f, 0.17f, 0.17f);
			pGameInstance->Add_DirCA(_float2(-1.f, 1.f), _float4(0.008f, 0.f, 0.004f, 0.f), 0.4f, 0.4f);
		);
	}


	Hit_Unit(m_pColCom, ELEMENTAL_FIRE, fRandom(1100.f, 1250.f), 56, 120, m_pTargetModel->Get_Animation()[m_pState_Handler->Get_AnimationIdx(m_eState)], &m_bHit0, "Player_Hit_ShieldAxe");
	...
	if (m_bHit0)
	{
		...
		IMPL_ONCE(m_bGauge0, m_pPlayer->Set_SkillGauge(fRandom(1100.f, 1250.f)));
	}
	...
	if (m_pState_Handler->Get_KeyInput() && 120 < Get_CurKeyFrame())
	{
		m_bEndCurState = true;
	}
...
void CState_Player_Shield_Skill_Normal::Exit(void* pArg)
{
	m_pColCom = ((CCollider*)m_pState_Handler->Get_Player()->Get_Component(ID_UINT(COLLIDER, COLLIDER_SPHERE, COL_PLAYER_ATTACK)));
	CTransform* pPlayerTransform = (CTransform*)m_pState_Handler->Get_Player()->Get_Component(TRANSFORM);
...
_bool CState_Player_Shield_Skill_Normal::Check_Condition(CState_Handler* pHandler)
{
	USEGAMEINSTANCE;
	if (KEY_DOWN('1'))
	{
...
CState_Player_Shield_Skill_Normal* CState_Player_Shield_Skill_Normal::Create(CState_Manager* pManager, STATE_TYPE eState)
{
	CState_Player_Shield_Skill_Normal* pInstance = new CState_Player_Shield_Skill_Normal(pManager, eState);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created : CState_Player_Shield_Skill_Normal");
...
CState* CState_Player_Shield_Skill_Normal::Clone(void* pArg)
{
	CState_Player_Shield_Skill_Normal* pInstance = new CState_Player_Shield_Skill_Normal(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned : CState_Player_Shield_Skill_Normal");
		Safe_Release(pInstance);
	}
...
void CState_Player_Shield_Skill_Normal::Free()
{
}
