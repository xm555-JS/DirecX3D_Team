#include "stdafx.h"
#include "..\Public\State_Player_Axe_Roll_Back.h"
...
CState_Player_Axe_Roll_Back::CState_Player_Axe_Roll_Back(CState_Manager* pManager, STATE_TYPE eState)
	: CState_Player(pManager, eState)
{
}

CState_Player_Axe_Roll_Back::CState_Player_Axe_Roll_Back(const CState_Player_Axe_Roll_Back& rhs)
	: CState_Player(rhs)
{
}

HRESULT CState_Player_Axe_Roll_Back::Initialize_Prototype()
{
...
HRESULT CState_Player_Axe_Roll_Back::Initialize(void* pArg)
{
	return S_OK;
}

void CState_Player_Axe_Roll_Back::Enter(void* pArg)
{
	__super::Enter(pArg);
...
void CState_Player_Axe_Roll_Back::Tick(_float fTimeDelta)
{
	USEGAMEINSTANCE;
...
void CState_Player_Axe_Roll_Back::Exit(void* pArg)
{
	m_fKeyPressTimeAcc = 0.f;
	CTransform* pPlayerTransform = (CTransform*)m_pState_Handler->Get_Player()->Get_Component(TRANSFORM);
...
_bool CState_Player_Axe_Roll_Back::Check_Condition(CState_Handler* pHandler)
{
	USEGAMEINSTANCE;
...
CState_Player_Axe_Roll_Back* CState_Player_Axe_Roll_Back::Create(CState_Manager* pManager, STATE_TYPE eState)
{
	CState_Player_Axe_Roll_Back* pInstance = new CState_Player_Axe_Roll_Back(pManager, eState);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created : CState_Player_Axe_Roll_Back");
...
CState* CState_Player_Axe_Roll_Back::Clone(void* pArg)
{
	CState_Player_Axe_Roll_Back* pInstance = new CState_Player_Axe_Roll_Back(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned : CState_Player_Axe_Roll_Back");
		Safe_Release(pInstance);
	}
...
void CState_Player_Axe_Roll_Back::Free()
{
}
