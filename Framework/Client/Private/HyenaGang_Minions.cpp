#include "stdafx.h"
#include "..\Public\HyenaGang_Minions.h"
...
/*하이에나 졸개*/
CHyenaGang_Minions::CHyenaGang_Minions()
	: CEnemy_Normal()
{
}

CHyenaGang_Minions::CHyenaGang_Minions(const CHyenaGang_Minions & rhs)
	: CEnemy_Normal(rhs)
{
}

HRESULT CHyenaGang_Minions::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CHyenaGang_Minions::Initialize(void * pArg)
{
	m_fFrustumRange = 2.f;
...
void CHyenaGang_Minions::Tick(_float fTimeDelta)
{
#if defined(_TEST_JJ)
	DeadTestKey();
#endif
...
void CHyenaGang_Minions::LateTick(_float fTimeDelta)
{
#ifndef _MAPTOOL
...
HRESULT CHyenaGang_Minions::Render()
{
...
void CHyenaGang_Minions::On_Dead()
{
	USEGAMEINSTANCE;
...
HRESULT CHyenaGang_Minions::SetUp_Components()
{
	FAILEDCHECK(__super::Add_Component(LEVEL_STATIC, ID_UINT(SHADER, SHADER_VTXANIM), ID_UINT(SHADER), (CComponent**)&m_pShaderCom));
	FAILEDCHECK(__super::Add_Component(LEVEL_GAMEPLAY, ID_UINT(MODEL_DYNAMIC, MODEL_ENEMY, HYENAGANG_MINIONS), ID_UINT(MODEL_DYNAMIC), (CComponent**)&m_pModelCom));
...
HRESULT CHyenaGang_Minions::SetUp_State_Handler()
{
...
HRESULT CHyenaGang_Minions::SetUp_Weapon()
{
	USEGAMEINSTANCE;
...
CHyenaGang_Minions * CHyenaGang_Minions::Create()
{
	CHyenaGang_Minions*		pInstance = new CHyenaGang_Minions();

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created : CHyenaGang_Minions");		
...
CGameObject * CHyenaGang_Minions::Clone(void * pArg)
{
	CHyenaGang_Minions*		pInstance = new CHyenaGang_Minions(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned : CHyenaGang_Minions");
		Safe_Release(pInstance);
	}
