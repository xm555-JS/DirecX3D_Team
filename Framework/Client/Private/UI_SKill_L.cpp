...
CUI_Skill_L::CUI_Skill_L()
	: CUI()
{
}

CUI_Skill_L::CUI_Skill_L(const CUI_Skill_L & rhs)
	: CUI(rhs)
{
}

HRESULT CUI_Skill_L::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CUI_Skill_L::Initialize(void * pArg)
{
	...
	m_pShortKey = (CUI*)pGameInstance->Add_GameObject(m_iCurLevel, ID_UINT(UI,UI_SHORTKEY), ID_UINT(UI, UI_SHORTKEY),
		&UIDESC(1130.f, 610.f, 17.5f, 17.5f, 0.f, 9.f, 299.f));

	m_pSkillTime = (CUI*)pGameInstance->Add_GameObject(m_iCurLevel, ID_UINT(UI, UI_SKILL_TIME), ID_UINT(UI, UI_SKILL_TIME),
		&UIDESC(1097.f, 586.f, 34.f, 47.5f, 0.f, 22, 299.f));

	return S_OK;
}

void CUI_Skill_L::Tick(_float fTimeDelta)
{	
	CUI::Tick(fTimeDelta);
	SetUp_SkillIcon();
	...
	if (pPlayer)
	{
		m_iSkillTime = pPlayer->Get_TimeChangeWeapon(WPN_SLOT_L);
...
void CUI_Skill_L::LateTick(_float fTimeDelta)
{
	GAMEINSTANCE->Add_RenderGroup(RENDER_UI, this);
}

HRESULT CUI_Skill_L::Render()
{
...
void CUI_Skill_L::On_EventMessege(EVENTDESC tEvent)
{
	if (EVENT_SKILLCHANGE_L == tEvent.eEventType)
	{
		m_bChange = true;
		m_fAccTime = 0.f;
		m_fDir = 1.f;
...
void CUI_Skill_L::SetUp_SkillIcon()
{
	USEGAMEINSTANCE;
	CPlayer* pPlayer = (CPlayer*)PLAYERPTR;
	...
	if (pPlayer && m_pShortKey)
	{
		...
		_uint iKey = pPlayer->Get_Slot_WeaponKey(WPN_SLOT_L);
		switch (iKey)
		{
		case 'Q':
...
void CUI_Skill_L::Change_SkillAnim(_float fTimeDelta)
{
	if (m_bChange)
	{
...
HRESULT CUI_Skill_L::SetUp_Components()
{
	/* For.Com_Shader */
...
HRESULT CUI_Skill_L::SetUp_ShaderResource()
{
...
CUI_Skill_L * CUI_Skill_L::Create()
{
	CUI_Skill_L*		pInstance = new CUI_Skill_L();

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created : CUI_Skill_L");		
...
CGameObject * CUI_Skill_L::Clone(void * pArg)
{
	CUI_Skill_L*		pInstance = new CUI_Skill_L(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned : CUI_Skill_L");
		Safe_Release(pInstance);
	}
...
void CUI_Skill_L::Free()
{
	USEGAMEINSTANCE;
	__super::Free();
	pGameInstance->Remove_CallbackList(this, EVENT_SKILLCHANGE_L);
}
